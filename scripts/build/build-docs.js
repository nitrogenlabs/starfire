...
const starfirePath = isPullRequest ? "dist" : "node_modules/starfire/";

const parserPaths = parsers.map(stripLanguageDirectory);

// --- Build starfire for PR ---
...
shell.cp(`${starfirePath}/index.js`, `${docs}/index.js`);
shell.exec(
...
for(const parser of parserPaths) {
  ...
  shell.exec(
    `rollup -c scripts/build/rollup.docs.config.js --environment filepath:${parser}.js -i ${starfirePath}/${parser}.js`
  );
}
...
shell.echo("Copy starfire-animated-logo CSS file to docs");
shell.cp(
  path.join(
    rootDir,
    "website/node_modules/@sandhose/starfire-animated-logo/dist/wide.css"
  ),
  `${docs}/starfire-animated-logo.css`
);
