...
const starfirePkg = require(path.join(starfireRootDir, "package.json"));
const starfireCli = path.join(starfireRootDir, starfirePkg.bin.starfire);

const thirdParty = isProduction
  ? path.join(starfireRootDir, "./third-party")
  : path.join(starfireRootDir, "./src/common/third-party");

function runStarfire(dir, args, options) {
  args = args || [];
  options = options || {};
  ...
  process.argv = ["path/to/node", "path/to/starfire/bin"].concat(args);
