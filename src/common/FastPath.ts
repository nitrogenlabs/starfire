...
export class FastPath {
  ...
  static getNodeHelper(path, count) {
    const s = path.stack;
  ...
  getNode(count: number = 0) {
    return FastPath.getNodeHelper(this, ~~count);
  }

  getParentNode(count: number = 0) {
    return FastPath.getNodeHelper(this, ~~count + 1);
  }
  ...
  needsParens(options): boolean {
    ...
    if(FastPath.isStatement(node)) {
      return false;
    }

    // Closure compiler requires that type casted expressions to be surrounded by
    // parentheses.
    if(Util.hasClosureCompilerTypeCastComment(options.originalText, node, options.locEnd)) {
      return true;
    }
    ...
    if(
      (parent.type === 'ArrowFunctionExpression' &&
        ...
        Util.startsWithNoLookaheadToken(node, /* forbidFunctionAndClass */ false)) ||
    ...
    switch(node.type) {
      ...
      case 'BinaryExpression': {
        ...
        const isLeftOfAForStatement = (nodeItem): boolean => {
          let i: number = 0;

          while(nodeItem) {
            const parentNode = this.getParentNode(i++);

            if(!parentNode) {
              return false;
            }

            if(parentNode.type === 'ForStatement' && parentNode.init === nodeItem) {
              return true;
            }
  ...
  static isStatement(node): boolean {
    return (
