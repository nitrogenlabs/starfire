...
export class PrinterMarkdown {
    ...
    if(PrinterMarkdown.shouldRemainTheSameContent(path)) {
      return concat(
        Util
          ...
          .map(
            (splitTextNode) =>
              splitTextNode.type === 'word'
                ? splitTextNode.value
                : splitTextNode.value === '' ? '' : PrinterMarkdown.printLine(path, splitTextNode.value, options)
          )
      );
    }
  ...
  static printTitle(title, options) {
    if(!title) {
      return '';
    }
