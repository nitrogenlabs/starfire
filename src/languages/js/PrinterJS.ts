...
import {CommentsJS} from './CommentsJS';
...
const isIdentifierName = ESUtils.keyword.isIdentifierNameES6;

export class PrinterJS {
  static handleComments = {
    endOfLine: CommentsJS.handleEndOfLineComment,
    ownLine: CommentsJS.handleOwnLineComment,
    remaining: CommentsJS.handleRemainingComment
  };
  static insertPragma = PragmaJS.insertPragma;
  static massageAstNode = CleanJS.clean;
  static print = PrinterJS.genericPrint;

  static shouldPrintComma(options, level: string = 'es5'): boolean {
            ...
            (decorator.type === 'CallExpression' &&
              (decorator.arguments.length === 0 ||
                (decorator.arguments.length === 1 &&
                  (PrinterJS.isStringLiteral(decorator.arguments[0]) ||
  ...
  static hasStarfireIgnore(path) {
    return Util.hasIgnoreComment(path) || PrinterJS.hasJsxIgnoreComment(path);
  }

  static hasJsxIgnoreComment(path) {
    const node = path.getValue();
    const parent = path.getParentNode();
    if(!parent || !node || !PrinterJS.isJSXNode(node) || !PrinterJS.isJSXNode(parent)) {
      return false;
    }
    ...
    for(let i = index; i > 0; i--) {
      ...
      if(candidate.type === 'JSXText' && !PrinterJS.isMeaningfulJSXText(candidate)) {
        continue;
      }
    ...
    return (
      ...
      prevSibling.expression.comments.find(
        (comment) => comment.value.trim() === 'starfire-ignore'
      )
    );
  }
  ...
  static formatTernaryOperator(path, options, print, operatorOptions) {
    ...
    if(
      (operatorOpts.shouldCheckJsx && PrinterJS.isJSXNode(n[operatorOpts.testNode])) ||
      PrinterJS.isJSXNode(n[operatorOpts.consequentNode]) ||
      PrinterJS.isJSXNode(n[operatorOpts.alternateNode]) ||
      PrinterJS.conditionalExpressionChainContainsJSX(lastConditionalParent)
    ) {
      jsxMode = true;
      forceNoIndent = true;
  ...
  static printPathNoParens(path, options, print, args) {
    ...
    switch(n.type) {
      ...
      case 'Program':
        ...
        parts.push(path.call((bodyPath) => PrinterJS.printStatementSequence(bodyPath, options, print), 'body'));
      ...
      case 'ExpressionStatement':
        ...
        if(n.directive) {
          return concat([PrinterJS.nodeStr(n.expression, options, true), semi]);
        }
        // Do not append semicolon after the only JSX element in a program
        return concat([
          path.call(print, 'expression'),
          PrinterJS.isTheOnlyJSXElementInMarkdown(options, path) ? '' : semi
        ]); // Babel extension.
      case 'ParenthesizedExpression':
        return concat(['(', path.call(print, 'expression'), ')']);
      case 'AssignmentExpression':
        return PrinterJS.printAssignment(
          n.left,
      ...
      case 'LogicalExpression': {
        ...
        const updatedParts = PrinterJS.printBinaryishExpressions(path, print, options, false, isInsideParenthesis);
        ...
        //     c
        //   ).call()
        if(parent.type === 'UnaryExpression' || (parent.type === 'MemberExpression' && !parent.computed)) {
          return group(concat([indent(concat([softline, concat(updatedParts)])), softline]));
        }
        ...
        const samePrecedenceSubExpression = PrinterJS.isBinaryish(n.left)
          && Util.shouldFlatten(n.operator, n.left.operator);

        if(
          shouldNotIndent ||
          (PrinterJS.shouldInlineLogicalExpression(n) && !samePrecedenceSubExpression) ||
          (!PrinterJS.shouldInlineLogicalExpression(n) && shouldIndentIfInlining)
        ) {
          return group(concat(updatedParts));
        }

        const rest = concat(updatedParts.slice(1));
        ...
        return group(
          concat([
            ...
            updatedParts.length > 0 ? updatedParts[0] : '',
            indent(rest)
          ])
        );
      }
      ...
      case 'TSTypeAssertionExpression':
        return concat(['<', path.call(print, 'typeAnnotation'), '>', path.call(print, 'expression')]);
      case 'MemberExpression': {
        ...
        return concat([
          ...
          shouldInline
            ? PrinterJS.printMemberLookup(path, options, print)
            : group(
              indent(
                concat([softline, PrinterJS.printMemberLookup(path, options, print)])
              )
            )
        ]);
      }
      ...
      case 'BindExpression':
        ...
        parts.push(PrinterJS.printBindExpressionCallee(path, options, print));
        return concat(parts);
      case 'Identifier': {
        return concat([
          n.name,
      ...
      case 'ObjectTypeSpreadProperty':
        return concat(['...', path.call(print, 'argument'), PrinterJS.printTypeAnnotation(path, options, print)]);
      ...
      case 'FunctionExpression':
        ...
        parts.push(PrinterJS.printFunctionDeclaration(path, print, options));
      ...
      case 'ArrowFunctionExpression': {
        ...
        if(PrinterJS.shouldPrintParamsWithoutParens(path, options)) {
          parts.push(path.call(print, 'params', 0));
        } else {
          parts.push(
            group(
              concat([
                PrinterJS.printFunctionParams(
                  path,
                  print,
                  options,
                /* expandLast */ args &&
                ...
                PrinterJS.printReturnType(path, print, options)
              ])
            )
          );
        }
        ...
        const printTrailingComma =
          args && args.expandLastArg && PrinterJS.shouldPrintComma(options, 'all');
      ...
      case 'TSAbstractMethodDefinition':
        ...
        parts.push(PrinterJS.printMethod(path, options, print));
      ...
      case 'ExportNamedDeclaration':
        return PrinterJS.printExportDeclaration(path, options, print);
      ...
      case 'BlockStatement': {
        const naked = path.call((bodyPath) => {
          return PrinterJS.printStatementSequence(bodyPath, options, print);
        }, 'body');
        ...
        if(
          !hasContent &&
          !hasDirectives &&
          !PrinterJS.hasDanglingComments(n) &&
      ...
      case 'ReturnStatement':
        ...
        if(n.argument) {
          if(PrinterJS.returnArgumentHasLeadingComment(options, n.argument)) {
            parts.push(
              concat([
                ' (',
        ...
        if(PrinterJS.hasDanglingComments(n)) {
          parts.push(
            ' ',
      ...
      case 'CallExpression': {
        ...
        const optional = PrinterJS.printOptionalToken(path);
        if(
        ...
        if(!isNew && PrinterJS.isMemberish(n.callee)) {
          return PrinterJS.printMemberChain(path, options, print);
        }
        ...
        return concat([
          ...
          path.call(print, 'callee'),
          optional,
          PrinterJS.printFunctionTypeParameters(path, options, print),
          PrinterJS.printArgumentsList(path, options, print)
        ]);
      }
      case 'TSInterfaceDeclaration':
        ...
        parts.push(
          n.abstract ? 'abstract ' : '',
          PrinterJS.printTypeScriptModifiers(path, options, print),
          'interface ',
          path.call(print, 'id'),
      ...
      case 'TSTypeLiteral': {
        ...
        if(
          (n.type === 'ObjectPattern' &&
            parent &&
            PrinterJS.shouldHugArguments(parent) &&
            parent.params[0] === n) ||
          (PrinterJS.shouldHugType(n) &&
            parentParentParent &&
            PrinterJS.shouldHugArguments(parentParentParent) &&
      ...
      case 'Property':
        if(n.method || n.kind === 'get' || n.kind === 'set') {
          return PrinterJS.printMethod(path, options, print);
        }
        ...
        if(n.shorthand) {
          ...
          if(n.computed) {
            ...
            printedLeft = PrinterJS.printPropertyKey(path, options, print);
          }
          parts.push(
            PrinterJS.printAssignment(
              n.key,
              printedLeft,
              ':',
              n.value,
      ...
      case 'ClassMethod':
        ...
        parts = parts.concat(PrinterJS.printObjectMethod(path, options, print));

        return concat(parts); // Babel 6
      case 'ObjectMethod':
        return PrinterJS.printObjectMethod(path, options, print);
      case 'Decorator':
        return concat(['@', path.call(print, 'expression')]);
      ...
      case 'ArrayPattern':
        if(n.elements.length === 0) {
          ...
          const needsForcedTrailingComma: boolean = canHaveTrailingComma && lastElem === null;
      ...
      case 'SequenceExpression': {
        ...
        if(
          ...
          path.each((p) => {
            if(p.getName() === 0) {
              updatedParts.push(print(p));
            } else {
              updatedParts.push(',', indent(concat([line, print(p)])));
            }
          }, 'expressions');
          return group(concat(updatedParts));
        }
      ...
      case 'Literal': {
        if(n.regex) {
          return PrinterJS.printRegex(n.regex);
        }
      ...
      case 'DirectiveLiteral':
        return PrinterJS.nodeStr(n, options);
      ...
      case 'ConditionalExpression':
        return PrinterJS.formatTernaryOperator(path, options, print, {
      ...
      case 'VariableDeclaration': {
        ...
        parts = [
          PrinterJS.isNodeStartingWithDeclare(n, options) ? 'declare ' : '',
          n.kind,
          firstVariable ? concat([' ', firstVariable]) : '',
          indent(
            concat(printed.slice(1).map((p) => concat([',', hasValue && !isParentForLoop ? hardline : line, p])))
          )
        ];
      ...
      case 'IfStatement': {
        ...
        if(n.alternate) {
          ...
          parts.push(
            group(
              PrinterJS.adjustClause(n.alternate, path.call(print, 'alternate'), n.alternate.type === 'IfStatement')
            )
          );
        }
      ...
      case 'ForStatement': {
        const body = PrinterJS.adjustClause(n.body, path.call(print, 'body'));
      ...
      case 'DoWhileStatement': {
        const clause = PrinterJS.adjustClause(n.body, path.call(print, 'body'));
      ...
      case 'SwitchCase': {
        ...
        if(consequent.length > 0) {
          const cons = path.call((consequentPath) => {
            return PrinterJS.printStatementSequence(consequentPath, options, print);
          }, 'consequent');
      ...
      case 'JSXAttribute':
        ...
        if(n.value) {
          let res;
          if(PrinterJS.isStringLiteral(n.value)) {
      ...
      case 'JSXSpreadChild': {
        return concat([
          ...
          path.call((p) => {
            ...
            const nodeValue = p.getValue();
            if(!nodeValue.comments || !nodeValue.comments.length) {
              return printed;
            }
            return concat([
              indent(
      ...
      case 'JSXExpressionContainer': {
        ...
        const shouldInline =
          ...
          (n.expression.type === 'ArrayExpression' ||
            ...
            (PrinterJS.isJSXNode(parent) &&
              (n.expression.type === 'ConditionalExpression' || PrinterJS.isBinaryish(n.expression))));
      ...
      case 'TSJsxFragment':
      case 'JSXElement': {
        const elem = Comments.printComments(path, () => PrinterJS.printJSXElement(path, options, print), options);
        return PrinterJS.maybeWrapJSXElementInParens(path, elem);
      }
      case 'JSXOpeningElement': {
        const nodeValue = path.getValue();
        const nameHasComments = nodeValue.name && nodeValue.name.comments && nodeValue.name.comments.length > 0;

        // Don't break self-closing elements with no attributes and no comments
        if(nodeValue.selfClosing && !nodeValue.attributes.length && !nameHasComments) {
        ...
        if(
          ...
          nodeValue.attributes.length === 1 &&
          nodeValue.attributes[0].value &&
          PrinterJS.isStringLiteral(n.attributes[0].value) &&
          !nodeValue.attributes[0].value.value.includes('\n') &&
          ...
          // >
          !nameHasComments &&
          (!nodeValue.attributes[0].comments || !nodeValue.attributes[0].comments.length)
        ) {
          return group(
            concat([
              '<',
              ...
              nodeValue.selfClosing ? ' />' : '>'
            ])
          );
        }

        const lastAttrHasTrailingComments = n.attributes.length
      ...
      case 'ClassPrivateProperty': {
        ...
        const variance = PrinterJS.getFlowVariance(n);
        if(variance) {
          parts.push(variance);
        }
        if(n.computed) {
          ...
          parts.push(PrinterJS.printPropertyKey(path, options, print));
        }
        parts.push(PrinterJS.printTypeAnnotation(path, options, print));
        if(n.value) {
          parts.push(' =', PrinterJS.printAssignmentRight(n.value, path.call(print, 'value'), false, options));
        }
      ...
      case 'TSAbstractClassDeclaration':
        ...
        parts.push(concat(PrinterJS.printClass(path, options, print)));
        return concat(parts);
        ...
        return group(
          concat([
            ...
            indent(
              concat([
                softline,
                PrinterJS.printArrayItems(path, options, typesField, print)
              ])
            ),
      ...
      case 'BooleanLiteralTypeAnnotation':
        return '' + n.value;
      case 'DeclareClass':
        return PrinterJS.printFlowDeclaration(path, PrinterJS.printClass(path, options, print));
      case 'DeclareFunction':
        ...
        if(n.params) {
          return concat([
            'declare ',
            PrinterJS.printFunctionDeclaration(path, print, options),
            semi
          ]);
        }
        return PrinterJS.printFlowDeclaration(path, [
          'function ',
          path.call(print, 'id'),
          n.predicate ? ' ' : '',
      ...
      case 'DeclareVariable':
        return PrinterJS.printFlowDeclaration(path, ['var ', path.call(print, 'id'), semi]);
      case 'DeclareExportAllDeclaration':
        return concat(['declare export * from ', path.call(print, 'source')]);
      case 'DeclareExportDeclaration':
        return concat(['declare ', PrinterJS.printExportDeclaration(path, options, print)]);
      ...
      case 'OpaqueType': {
        ...
        if(n.type === 'DeclareOpaqueType') {
          return PrinterJS.printFlowDeclaration(path, parts);
        }
      ...
      case 'TSFunctionType': {
        ...
        if(PrinterJS.isObjectTypePropertyAFunction(parent, options)) {
        ...
        parts.push(PrinterJS.printFunctionParams(path, print, options, false, true));
      ...
      case 'InterfaceDeclaration': {
        if(
          ...
          PrinterJS.isNodeStartingWithDeclare(n, options)
        ) {
          parts.push('declare ');
        }
      ...
      case 'IntersectionTypeAnnotation': {
        ...
        for(let i = 0; i < types.length; ++i) {
          if(i === 0) {
            result.push(types[i]);
          } else if(PrinterJS.isObjectType(n.types[i - 1]) && PrinterJS.isObjectType(n.types[i])) {
      ...
      case 'UnionTypeAnnotation': {
        ...
        const shouldHug = PrinterJS.shouldHugType(n);
      ...
      case 'ObjectTypeProperty': {
        ...
        return concat([
          n.static ? 'static ' : '',
          PrinterJS.isGetterOrSetter(n) ? n.kind + ' ' : '',
          variance || '',
          PrinterJS.printPropertyKey(path, options, print),
          PrinterJS.printOptionalToken(path),
          PrinterJS.isFunctionNotation(n, options) ? '' : ': ',
          path.call(print, 'value')
        ]);
      }
      ...
      case 'TypeAlias': {
        ...
        const printed = PrinterJS.printAssignmentRight(n.right, path.call(print, 'right'), canBreak, options);
      ...
      case 'TSTypeParameterInstantiation':
        return PrinterJS.printTypeParameters(path, options, print, 'params');
      ...
      case 'TSPropertySignature': {
        ...
        parts.push(PrinterJS.printOptionalToken(path));
      ...
      case 'TSTypeReference':
        return concat([
          ...
          PrinterJS.printTypeParameters(path, options, print, 'typeParameters')
        ]);
      case 'TSTypeQuery':
      ...
      case 'TSEnumDeclaration':
        ...
        if(n.modifiers) {
          parts.push(PrinterJS.printTypeScriptModifiers(path, options, print));
        }
        if(n.const) {
          parts.push('const ');
        }
            ...
            group(
              concat([
                ...
                indent(
                  concat([
                    hardline,
                    PrinterJS.printArrayItems(path, options, 'members', print),
                    PrinterJS.shouldPrintComma(options, 'es5') ? ',' : ''
                  ])
                ),
                Comments.printDanglingComments(
                  path,
      ...
      case 'TSModuleDeclaration': {
        const parent = path.getParentNode();
        const isExternalModule = PrinterJS.isLiteral(n.id);
  ...
  static printStatementSequence(path, options, print) {
    const printed = [];
    ...
    path.map((stmtPath, i) => {
      ...
      if(
        !options.semi &&
        !isClass &&
        !PrinterJS.isTheOnlyJSXElementInMarkdown(options, stmtPath) &&
        PrinterJS.stmtNeedsASIProtection(stmtPath, options)
      ) {
      ...
      if(!options.semi && isClass) {
        if(PrinterJS.classPropMayCauseASIProblems(stmtPath)) {
          parts.push(';');
        } else if(stmt.type === 'ClassProperty') {
          const nextChild = bodyNode.body[i + 1];
          if(PrinterJS.classChildNeedsASIProtection(nextChild)) {
            parts.push(';');
          }
        }
      }
      ...
      if(
        ...
        !PrinterJS.isLastStatement(stmtPath)
      ) {
        parts.push(hardline);
      }
  ...
  static printPropertyKey(path, options, print) {
    const node = path.getNode();
    const key = node.key;

    if(
      PrinterJS.isStringLiteral(key) &&
  ...
  static printMethod(path, options, print) {
    const node = path.getNode();
    ...
    let key = PrinterJS.printPropertyKey(path, options, print);
    ...
    parts.push(
      ...
      concat(
        path.call(
          (valuePath) => [
            PrinterJS.printFunctionTypeParameters(valuePath, options, print),
            group(
              concat([
                PrinterJS.printFunctionParams(valuePath, print, options),
                PrinterJS.printReturnType(valuePath, print, options)
              ])
            )
          ],
          'value'
    ...
    return (
      ...
      (arg.type === 'ArrowFunctionExpression' &&
        (arg.body.type === 'BlockStatement' ||
          ...
          PrinterJS.isJSXNode(arg.body)))
    );
  }

  static shouldGroupLastArg(args) {
    const lastArg = Util.getLast(args);
    const penultimateArg = Util.getPenultimate(args);
    return (
      !PrinterJS.hasLeadingComment(lastArg) &&
      !PrinterJS.hasTrailingComment(lastArg) &&
      PrinterJS.couldGroupArg(lastArg) &&
  ...
  static shouldGroupFirstArg(args) {
    if(args.length !== 2) {
      return false;
    }
    ...
    return (
      ...
      !PrinterJS.couldGroupArg(secondArg)
    );
  }

  static printArgumentsList(path, options, print) {
    const args = path.getValue().arguments;

    if(args.length === 0) {
      return concat(['(', Comments.printDanglingComments(path, options, /* sameIndent */ true), ')']);
    }
    ...
    const shouldGroupFirst = PrinterJS.shouldGroupFirstArg(args);
    const shouldGroupLast = PrinterJS.shouldGroupLastArg(args);
    if(shouldGroupFirst || shouldGroupLast) {
      ...
      const maybeTrailingComma = PrinterJS.shouldPrintComma(options, 'all') ? ',' : '';
    ...
    return group(
      concat([
        ...
        ifBreak(PrinterJS.shouldPrintComma(options, 'all') ? ',' : ''),
        softline,
        ')'
      ]),
  ...
  static printTypeAnnotation(path, options, print) {
    const node = path.getValue();
    ...
    if(PrinterJS.isFlowAnnotationComment(options.originalText, node.typeAnnotation, options)) {
      return concat([' /*: ', path.call(print, 'typeAnnotation'), ' */']);
    }
  ...
  static printFunctionTypeParameters(path, options, print) {
  ...
  static printFunctionParams(path, print, options, expandArg?, printTypeParams?) {
    const fun = path.getValue();
    ...
    //   c
    // }) {}
    if(PrinterJS.shouldHugArguments(fun)) {
    ...
    if(parent.type === 'CallExpression' && PrinterJS.isTestCall(parent)) {
      return concat([typeParams, '(', join(', ', printed), ')']);
    }
    ...
    const isFlowShorthandWithOneArg =
      (PrinterJS.isObjectTypePropertyAFunction(parent, options) ||
        PrinterJS.isTypeAnnotationAFunction(parent, options) ||
        parent.type === 'TypeAlias' ||
    ...
    return concat([
      ...
      ifBreak(
        canHaveTrailingComma && PrinterJS.shouldPrintComma(options, 'all') ? ',' : ''
      ),
      softline,
      ')'
    ]);
  ...
  static shouldPrintParamsWithoutParens(path, options): boolean {
    if(options.arrowParens === 'always') {
      return false;
    }
    ...
    if(options.arrowParens === 'avoid') {
      const node = path.getValue();
      return PrinterJS.canPrintParamsWithoutParens(node);
    }
  ...
  static canPrintParamsWithoutParens(node): boolean {
    return (
      node.params.length === 1 &&
      !node.rest &&
      !node.typeParameters &&
      !PrinterJS.hasDanglingComments(node) &&
  ...
  static printFunctionDeclaration(path, print, options) {
    const n = path.getValue();
    const parts = [];
    ...
    parts.push(
      ...
      group(
        concat([
          PrinterJS.printFunctionParams(path, print, options),
          PrinterJS.printReturnType(path, print, options)
        ])
      ),
      n.body ? ' ' : '',
  ...
  static printObjectMethod(path, options, print) {
    ...
    const key = PrinterJS.printPropertyKey(path, options, print);
  ...
  static printReturnType(path, print, options) {
    const n = path.getValue();
    const returnType = path.call(print, 'returnType');

    if(
      n.returnType &&
      PrinterJS.isFlowAnnotationComment(options.originalText, n.returnType, options)
    ) {
  ...
  static printFlowDeclaration(path, parts) {
  ...
  static printTypeScriptModifiers(path, options, print) {
    const n = path.getValue();
  ...
  static printTypeParameters(path, options, print, paramsKey) {
    const n = path.getValue();
    ...
    const isParameterInTestCall =
      ...
      PrinterJS.isTestCall(grandparent);

    const shouldInline =
      isParameterInTestCall ||
      n[paramsKey].length === 0 ||
      (n[paramsKey].length === 1 &&
        (PrinterJS.shouldHugType(n[paramsKey][0]) ||
          (n[paramsKey][0].type === 'GenericTypeAnnotation' &&
            PrinterJS.shouldHugType(n[paramsKey][0].id)) ||
          (n[paramsKey][0].type === 'TSTypeReference' &&
            PrinterJS.shouldHugType(n[paramsKey][0].typeName)) ||
    ...
    return group(
      concat([
        ...
        ifBreak(
          options.parser !== 'typescript' && PrinterJS.shouldPrintComma(options, 'all')
            ? ','
            : ''
        ),
        softline,
  ...
  static printClass(path, options, print) {
    const n = path.getValue();
    const parts = [];
  ...
  static printOptionalToken(path) {
    const node = path.getValue();
    if(!node.optional) {
      return '';
    }
  ...
  static printMemberLookup(path, options, print) {
    ...
    if(!n.property || PrinterJS.isNumericLiteral(n.property)) {
      return concat([optional, '[', property, ']']);
    }
  ...
  static printBindExpressionCallee(path, options, print) {
  ...
  static printMemberChain(path, options, print) {
    ...
    const shouldInsertEmptyLineAfter = (node) => {
    ...
    const rec = (pathValue) => {
      const nodeItem = pathValue.getValue();

      if(nodeItem.type === 'CallExpression' && (PrinterJS.isMemberish(nodeItem.callee)
        || nodeItem.callee.type === 'CallExpression')) {
        printedNodes.unshift({
          ...
          printed: concat([
            Comments.printComments(
              ...
              () =>
                concat([
                  PrinterJS.printOptionalToken(pathValue),
                  PrinterJS.printFunctionTypeParameters(pathValue, options, print),
                  PrinterJS.printArgumentsList(pathValue, options, print)
                ]),
              options
            ),
            shouldInsertEmptyLineAfter(nodeItem) ? hardline : ''
          ])
        });
        pathValue.call((callee) => rec(callee), 'callee');
      } else if(PrinterJS.isMemberish(nodeItem)) {
        printedNodes.unshift({
          node: nodeItem,
          printed: Comments.printComments(
            pathValue,
            () =>
              nodeItem.type === 'MemberExpression'
                ? PrinterJS.printMemberLookup(pathValue, options, print)
                : PrinterJS.printBindExpressionCallee(pathValue, options, print),
            options
          )
        });
        pathValue.call((object) => rec(object), 'object');
      } else if(nodeItem.type === 'TSNonNullExpression') {
        printedNodes.unshift({
          node: nodeItem,
          printed: Comments.printComments(pathValue, () => '!', options)
        });
        pathValue.call((expression) => rec(expression), 'expression');
      } else {
        printedNodes.unshift({node: nodeItem, printed: pathValue.call(print)});
      }
    };
    ...
    for(; i < printedNodes.length; ++i) {
      if(
        ...
        (printedNodes[i].node.type === 'MemberExpression' &&
          ...
          PrinterJS.isNumericLiteral(printedNodes[i].node.property))
      ) {
    ...
    if(printedNodes[0].node.type !== 'CallExpression') {
      for(; i + 1 < printedNodes.length; ++i) {
        if(
          PrinterJS.isMemberish(printedNodes[i].node) &&
          PrinterJS.isMemberish(printedNodes[i + 1].node)
        ) {
          currentGroup.push(printedNodes[i]);
        } else {
          break;
    ...
    for(; i < printedNodes.length; ++i) {
      if(hasSeenCallExpression && PrinterJS.isMemberish(printedNodes[i].node)) {
    ...
    const printGroup = (printedGroup) => {
      return concat(printedGroup.map((tuple) => tuple.printed));
    };

    const printIndentedGroup = (indentGroups): string => {
      if(indentGroups.length === 0) {
        return '';
      }

      return indent(group(concat([hardline, join(hardline, indentGroups.map(printGroup))])));
    };
    ...
    const flatGroups = groups
      .slice(0, cutoff)
      .reduce((res, groupItem) => res.concat(groupItem), []);

    const hasComment =
      flatGroups.slice(1, -1).some((nodeItem) => PrinterJS.hasLeadingComment(nodeItem.node)) ||
      flatGroups.slice(0, -1).some((nodeItem) => PrinterJS.hasTrailingComment(nodeItem.node)) ||
      (groups[cutoff] && PrinterJS.hasLeadingComment(groups[cutoff][0].node));
  ...
  static isEmptyJSXElement(node) {
    ...
    return PrinterJS.isLiteral(child) && !PrinterJS.isMeaningfulJSXText(child);
  }

  // Only space, newline, carriage return, and tab are treated as whitespace
  // inside JSX.
  static jsxWhitespaceChars = ' \n\r\t';
  static containsNonJsxWhitespaceRegex = new RegExp(`[^${PrinterJS.jsxWhitespaceChars}]`);
  static matchJsxWhitespaceRegex = new RegExp(`([${PrinterJS.jsxWhitespaceChars}]+)`);

  // Meaningful if it contains non-whitespace characters,
  // or it contains whitespace without a new line.
  static isMeaningfulJSXText(node) {
    return (
      PrinterJS.isLiteral(node) &&
      (PrinterJS.containsNonJsxWhitespaceRegex.test(rawText(node)) ||
        !/\n/.test(rawText(node)))
    );
  }

  static conditionalExpressionChainContainsJSX(node) {
    return Boolean(PrinterJS.getConditionalChainContents(node).find(PrinterJS.isJSXNode));
  }
  ...
  static getConditionalChainContents(node) {
    // Given this code:
    //
    ...
    const recurse = (recurseNode) => {
      if(recurseNode.type === 'ConditionalExpression') {
        recurse(recurseNode.test);
        recurse(recurseNode.consequent);
        recurse(recurseNode.alternate);
      } else {
        nonConditionalExpressions.push(recurseNode);
      }
    };
  ...
  static isJSXWhitespaceExpression(node) {
    return (
      node.type === 'JSXExpressionContainer' &&
      PrinterJS.isLiteral(node.expression) &&
  ...
  static printJSXChildren(path, options, print, jsxWhitespace) {
    const n = path.getValue();
    const children = [];
    ...
    path.map((childPath, i) => {
      ...
      if(PrinterJS.isLiteral(child)) {
        const text = rawText(child);

        // Contains a non-whitespace character
        if(PrinterJS.isMeaningfulJSXText(child)) {
          const words = text.split(PrinterJS.matchJsxWhitespaceRegex);
          ...
          if(endWhitespace !== undefined) {
            ...
            // (https://github.com/starfire/starfire/issues/1581#issuecomment-300975032)
            ...
            // will allow Facebook to adopt Starfire without too much of an
        ...
        const directlyFollowedByMeaningfulText =
          next && PrinterJS.isMeaningfulJSXText(next) && !/^[ \n\r\t]/.test(rawText(next));
  ...
  static printJSXElement(path, options, print) {
    const n = path.getValue();

    // Turn <div></div> into <div />
    if(n.type === 'JSXElement' && PrinterJS.isEmptyJSXElement(n)) {
    ...
    n.children = n.children.map((child) => {
      if(PrinterJS.isJSXWhitespaceExpression(child)) {
        return {
          raw: ' ',
          type: 'JSXText',
          value: ' '
    ...
    const containsTag = n.children.filter(PrinterJS.isJSXNode).length > 0;
    ...
    const children = PrinterJS.printJSXChildren(path, options, print, jsxWhitespace);

    const containsText =
      n.children.filter((child) => PrinterJS.isMeaningfulJSXText(child)).length > 0;
    ...
    while(
      children.length &&
      (isLineNext(Util.getLast(children)) ||
        isEmpty(Util.getLast(children)))
    ) {
      children.pop();
    }
  ...
  static maybeWrapJSXElementInParens(path, elem) {
    const parent = path.getParentNode();
  ...
  static isBinaryish(node) {
    return node.type === 'BinaryExpression' || node.type === 'LogicalExpression';
  }

  static isMemberish(node) {
    return (
  ...
  static shouldInlineLogicalExpression(node) {
    ...
    if(PrinterJS.isJSXNode(node.right)) {
      return true;
    }
  ...
  // broken before `+`.
  static printBinaryishExpressions(path, print, options, isNested, isInsideParenthesis) {
    let parts = [];
    const node = path.getValue();

    // We treat BinaryExpression and LogicalExpression nodes the same.
    if(PrinterJS.isBinaryish(node)) {
      ...
      if(Util.shouldFlatten(node.operator, node.left.operator)) {
        ...
        parts = parts.concat(
          path.call(
            (left) =>
              PrinterJS.printBinaryishExpressions(
                left,
                print,
                options,
              /* isNested */ true,
      ...
      const shouldInline = PrinterJS.shouldInlineLogicalExpression(node);
  ...
  static printAssignmentRight(rightNode, printedRight, canBreak, options) {
    if(PrinterJS.hasLeadingOwnLineComment(options.originalText, rightNode, options)) {
  ...
  static printAssignment(leftNode, printedLeft, operator, rightNode, printedRight, options) {
    if(!rightNode) {
      return printedLeft;
    }

    const canBreak =
      (PrinterJS.isBinaryish(rightNode) && !PrinterJS.shouldInlineLogicalExpression(rightNode)) ||
      (rightNode.type === 'ConditionalExpression' &&
        PrinterJS.isBinaryish(rightNode.test) &&
        !PrinterJS.shouldInlineLogicalExpression(rightNode.test)) ||
      ((leftNode.type === 'Identifier' ||
        PrinterJS.isStringLiteral(leftNode) ||
        leftNode.type === 'MemberExpression') &&
        (PrinterJS.isStringLiteral(rightNode) || PrinterJS.isMemberExpressionChain(rightNode)));

    const printed = PrinterJS.printAssignmentRight(rightNode, printedRight, canBreak, options);

    return group(concat([printedLeft, operator, printed]));
  }

  static adjustClause(node, clause, forceSpace: boolean = false) {
  ...
  static nodeStr(node, options, isFlowOrTypeScriptDirectiveLiteral?) {
  ...
  static hasLeadingComment(node) {
    return node.comments && node.comments.some((comment) => comment.leading);
  }

  static hasTrailingComment(node) {
    return node.comments && node.comments.some((comment) => comment.trailing);
  }

  static hasLeadingOwnLineComment(text, node, options) {
    if(PrinterJS.isJSXNode(node)) {
  ...
  static isFlowAnnotationComment(text, typeAnnotation, options) {
    const start = options.locStart(typeAnnotation);
    const end = Util.skipWhitespace(text, options.locEnd(typeAnnotation));
    return text.substr(start, 2) === '/*' && text.substr(end, 2) === '*/';
  }

  static getLeftSide(node) {
    if(node.expressions) {
  ...
  static getLeftSidePathName(path, node): any[] {
    if(node.expressions) {
    ...
    throw new PrintError('Unexpected node has no left side', node);
  }

  static exprNeedsASIProtection(path, options) {
    const node = path.getValue();
    ...
    const maybeASIProblem =
      ...
      (node.type === 'ArrowFunctionExpression' &&
        !PrinterJS.shouldPrintParamsWithoutParens(path, options)) ||
      node.type === 'ArrayExpression' ||
      node.type === 'ArrayPattern' ||
      (node.type === 'UnaryExpression' &&
        node.prefix && (node.operator === '+' || node.operator === '-')) ||
      node.type === 'TemplateLiteral' ||
      node.type === 'TemplateElement' ||
      PrinterJS.isJSXNode(node) ||
    ...
    if(!PrinterJS.hasNakedLeftSide(node)) {
      return false;
    }

    return path.call.apply(path, [(childPath) => PrinterJS.exprNeedsASIProtection(childPath, options)]
      .concat(PrinterJS.getLeftSidePathName(path, node)));
  }

  static stmtNeedsASIProtection(path, options) {
    const node = path.getNode();
    ...
    return path.call((childPath) => PrinterJS.exprNeedsASIProtection(childPath, options), 'expression');
  }

  static classPropMayCauseASIProblems(path) {
    const node = path.getNode();
  ...
  static returnArgumentHasLeadingComment(options, argument) {
    if(PrinterJS.hasLeadingOwnLineComment(options.originalText, argument, options)) {
      return true;
    }

    if(PrinterJS.hasNakedLeftSide(argument)) {
      let leftMost = argument;
      let newLeftMost;

      while((newLeftMost = PrinterJS.getLeftSide(leftMost))) {
        leftMost = newLeftMost;

        if(PrinterJS.hasLeadingOwnLineComment(options.originalText, leftMost, options)) {
          return true;
        }
      }
    }
  ...
  static isMemberExpressionChain(node) {
    ...
    return PrinterJS.isMemberExpressionChain(node.object);
  }

  // Hack to differentiate between the following two which have the same ast
  // type T = { method: () => void };
  // type T = { method(): void };
  static isObjectTypePropertyAFunction(node, options) {
    return (
      node.type === 'ObjectTypeProperty' &&
      node.value.type === 'FunctionTypeAnnotation' &&
      !node.static &&
      !PrinterJS.isFunctionNotation(node, options)
    );
  }

  // TODO: This is a bad hack and we need a better way to distinguish between
  // arrow functions and otherwise
  static isFunctionNotation(node, options) {
    return PrinterJS.isGetterOrSetter(node) || PrinterJS.sameLocStart(node, node.value, options);
  }

  static isGetterOrSetter(node) {
    return node.kind === 'get' || node.kind === 'set';
  }

  static sameLocStart(nodeA, nodeB, options) {
  ...
  // var f: (a) => void;
  static isTypeAnnotationAFunction(node, options) {
    return (
      (node.type === 'TypeAnnotation' || node.type === 'TSTypeAnnotation') &&
      node.typeAnnotation.type === 'FunctionTypeAnnotation' &&
      !node.static &&
      !PrinterJS.sameLocStart(node, node.typeAnnotation, options)
    );
  }

  static isNodeStartingWithDeclare(node, options) {
  ...
  static shouldHugType(node) {
    if(PrinterJS.isObjectType(node)) {
      return true;
    }
    ...
    return (
      ...
      (fun.params[0].type === 'ObjectPattern' ||
        ...
        (fun.params[0].type === 'Identifier' &&
          ...
          PrinterJS.isObjectType(fun.params[0].typeAnnotation.typeAnnotation)) ||
        (fun.params[0].type === 'FunctionTypeParam' &&
          PrinterJS.isObjectType(fun.params[0].typeAnnotation)) ||
  ...
  static templateLiteralHasNewLines(template) {
    return template.quasis.some((quasi) => quasi.value.raw.includes('\n'));
  }

  static isTemplateOnItsOwnLine(n, text, options) {
    return (
      ((n.type === 'TemplateLiteral' && PrinterJS.templateLiteralHasNewLines(n)) ||
        (n.type === 'TaggedTemplateExpression' &&
          PrinterJS.templateLiteralHasNewLines(n.quasi))) &&
      !Util.hasNewline(text, options.locStart(n), {backwards: true})
    );
  }

  static printArrayItems(path, options, printPath, print) {
  ...
  static hasDanglingComments(node) {
    return (
      node.comments &&
  ...
  static isTheOnlyJSXElementInMarkdown(options, path) {
    ...
    if(!node.expression || !PrinterJS.isJSXNode(node.expression)) {
      return false;
    }
  ...
  static willPrintOwnComments(path) {
    const node = path.getValue();
    const parent = path.getParentNode();

    return (
      ((node && PrinterJS.isJSXNode(node)) ||
        (parent &&
  ...
  static printComment(commentPath, options) {
    ...
    switch(comment.type) {
      case 'CommentBlock':
      case 'Block': {
        if(PrinterJS.isJsDocComment(comment)) {
          return PrinterJS.printJsDocComment(comment);
        }
  ...
  static isJsDocComment(comment) {
    ...
    return (
      lines.length > 1 &&
      lines.slice(0, lines.length - 1).every((lineItem) => lineItem.trim()[0] === '*')
    );
  }

  static printJsDocComment(comment) {
    const lines = comment.value.split('\n');
    ...
    return concat([
      ...
      join(
        ...
        lines.map((lineItem, index) => {
          return (index > 0 ? ' ' : '') + (index < lines.length - 1 ? lineItem.trim() : lineItem.trimLeft());
        })
      ),
      '*/'
    ]);
